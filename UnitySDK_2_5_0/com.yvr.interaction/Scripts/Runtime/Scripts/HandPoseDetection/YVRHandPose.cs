...
{
    public class YVRHandPose : MonoBehaviour
    {
        ...
        private void HandPoseEventCheck()
        {
            ...
            {
                case TrackType.Any:
                    poseStateActive = (leftShapesActive | rightShapesActive) & (leftBonesActive | rightBonesActive) &
                                      (leftTransActive | rightTransActive);
                    break;
                case TrackType.Left:
        ...
        private void Update()
        {
            ...
            {
                rightHandJointLocations = YVRHandManager.instance.rightHandData;

                for (int i = 0; i < rightJointPos.Count; ++i)
                {
                    if (rightHandJointLocations.jointLocations == null) break;

                    rightJointPos[i] = rightHandJointLocations.jointLocations[i].pose.position;

                    if (i == (int)HandJoint.JointWrist)
                    {
                        rightWirstPos = rightHandJointLocations.jointLocations[i].pose.position;
                        rightWirstRot = rightHandJointLocations.jointLocations[i].pose.orientation;
                    }
                }

                rightShapesHold = ShapesRecognizerCheck(rightJointPos, rightWirstRot * Vector3.left,
                    rightWirstRot * Vector3.back, rightWirstRot * Vector3.up);
                rightShapesActive = HoldCheck(rightShapesHold, shapesHoldDuration, rightShapesActive,
                    ref rightShapesHoldTime);

                rightBonesHold = BonesCheck(HandType.HandRight);
                rightBonesActive = HoldCheck(rightBonesHold, bonesHoldDuration, rightBonesActive,
                    ref rightBonesHoldTime);

                rightTransHold = TransCheck(rightWirstPos, rightWirstRot, HMDpose, rightTransHold);
                rightTransActive = HoldCheck(rightTransHold, transHoldDuration, rightTransActive,
                    ref rightTransHoldTime);
            }

            if (trackType == TrackType.Left || trackType == TrackType.Any)
            {
                leftHandJointLocations = YVRHandManager.instance.leftHandData;

                for (int i = 0; i < leftJointPos.Count; ++i)
                {
                    if (leftHandJointLocations.jointLocations == null) break;

                    leftJointPos[i] = leftHandJointLocations.jointLocations[i].pose.position;

                    if (i == (int)HandJoint.JointWrist)
                    {
                        leftWirstPos = leftHandJointLocations.jointLocations[i].pose.position;
                        leftWirstRot = leftHandJointLocations.jointLocations[i].pose.orientation;
                    }
                }

                leftShapesHold = ShapesRecognizerCheck(leftJointPos, leftWirstRot * Vector3.right,
                    leftWirstRot * Vector3.forward, leftWirstRot * Vector3.up);
                leftShapesActive = HoldCheck(leftShapesHold, shapesHoldDuration, leftShapesActive,
                    ref leftShapesHoldTime);
        ...
        private bool FlexionCheck(Finger finger, Vector3 wirstRight, Vector3 wirstForward,
            Vector3 wristUp)
        {
            if (finger.flexion == Flexion.Any) return true;
            else
            {
                float flexAngle = 0;
                ...
                return AngleCheck(flexAngle, finger.fingerConfigs.flexionConfigs.min,
                    finger.fingerConfigs.flexionConfigs.max, finger.fingerConfigs.flexionConfigs.width,
                    ShapesRecognizer.k_FlexionMin, ShapesRecognizer.k_FlexionMax);
            }
        }

        private bool CurlCheck(Finger finger)
        {
            if (finger.curl == Curl.Any) return true;
            else
            {
                float curlAngle = 0;
                switch (finger.handFinger)
                ...
                return AngleCheck(curlAngle, finger.fingerConfigs.curlConfigs.min, finger.fingerConfigs.curlConfigs.max,
                    finger.fingerConfigs.curlConfigs.width,
                    ShapesRecognizer.k_CurlMin, ShapesRecognizer.k_CurlMax);
            }
        }

        private bool AbductionCheck(Finger finger)
        {
            if (finger.abduction == Abduction.Any) return true;
            else
            {
                float abducAngle = 0;
                ...
                bool result = false;
                if (finger.abduction == Abduction.Open)
                {
                    result = AbducCheck(abducAngle, finger.fingerConfigs.abductionConfigs.mid,
                        finger.fingerConfigs.abductionConfigs.width);
                    ;
                }
                else if (finger.abduction == Abduction.Close)
                {
                    result = !AbducCheck(abducAngle, finger.fingerConfigs.abductionConfigs.mid,
                        finger.fingerConfigs.abductionConfigs.width);
                    ;
        ...
        private bool BonesCheck(HandType handType)
        {
            ...
            {
                float distance = Vector3.Distance(GetHandJoint(handType, bones[i].A_Bone),
                    GetHandJoint(handType, bones[i].B_Bone));
        ...
        private Vector3 GetHandJoint(HandType hand, HandJoint bone)
        {
            if (hand == HandType.HandLeft)
            {
                return leftHandJointLocations.jointLocations[(int)bone].pose.position;
            }
            else
            {
                return rightHandJointLocations.jointLocations[(int)bone].pose.position;
            }
        }
        ...
        {
            palmPos = wristRot * (trackType == TrackType.Right ? new Vector3(0.08f, 0, 0) : new Vector3(-0.08f, 0, 0)) +
                      wristPos;
